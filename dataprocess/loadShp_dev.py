...
def main():
	try:
		...
		for techType in os.listdir(rootDir):
		    ...
		    for company in os.listdir(techTypeDir):
		    	...
		    	for shapeFile in os.listdir(shapeFileDir):
					if shapeFile.endswith("shp") and shapeFile.find('_dissolved')>0:
						companyObj["fileName"]=shapeFile.replace("_dissolved.shp","")
						...
						if companyCount>331:
							...
							sqlComm = "insert into " + schema + "." + tableName + " select " + str(companyCount) + ", '" + company + "','" + techType + "', geom from " +  schema + "." + shapeFile.replace("_dissolved.shp","")
							print sqlComm
							cursor.execute(sqlComm)
							conn.commit()
							sqlComm = "drop table if EXISTS " + schema + "." + shapeFile.replace("_dissolved.shp","") + " CASCADE"
							cursor.execute(sqlComm)
							conn.commit()		
		print finalList
		...
		with open("c:\\projects\\python\\coverageRigh201210_dev.json" , 'w') as f:
			f.write(jsondata)
		f.closed
