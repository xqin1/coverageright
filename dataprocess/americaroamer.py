...
def main():
	try:
		    ...
		    for company in os.listdir(techTypeDir):
		    	...
		    	for shapeFile in os.listdir(shapeFileDir):
					if shapeFile.endswith("shp"):
						...
						print 'ogr2ogr "' + shapeFileDir + "\\" + shapeFile.split(".")[0] + '_ogr.shp" "'  + shapeFileDir + "\\" + shapeFile + '"'
						p1 = subprocess.Popen('ogr2ogr "' + shapeFileDir + "\\" + shapeFile.split(".")[0] + '_ogr.shp" "'  + shapeFileDir + "\\" + shapeFile + '"', stdout=subprocess.PIPE)
						p1.wait()
						...
						p.wait()
						if companyCount>50:
							shpComm = 'shp2pgsql -s 4326 -d -I -W latin1 -g geom "' + shapeFileDir + "\\" + shapeFile.split(".")[0] + '_ogr.shp" ' + schema + "." + shapeFile.split(".")[0] + " | psql -h " + host + " -d " + dbname + " -U " + user
							#print shpComm
							#shpP = subprocess.Popen(shpComm, stdout=None)
							print "load " + shapeFile + " to postgres"
							shpP = subprocess.call(shpComm,shell=True)
							sqlComm = "drop sequence if exists bseq"
							cursor.execute(sqlComm)
							conn.commit()
							sqlComm = "create sequence bseq"
							cursor.execute(sqlComm)
							conn.commit()
							sqlComm = "WITH ordered AS (SELECT geom FROM " + schema + "." + shapeFile.split(".")[0] + " ORDER BY ST_GeoHash(geom)),"
							sqlComm = sqlComm + "grouped AS (SELECT nextval('bseq') / 5000 AS id, ST_Union(geom) AS geom FROM ordered GROUP BY id)"
							sqlComm = sqlComm + "insert into " + schema + "." + tableName + " select " + str(companyCount) + ", '" + company + "','" + techType + "', st_multi(st_union(geom)) from grouped"
							print sqlComm
							cursor.execute(sqlComm)
							conn.commit()
							sqlComm = "drop sequence bseq"
							cursor.execute(sqlComm)
							conn.commit()
		...
		with open("c:\\projects\\python\\coverageRigh201210.json" , 'w') as f:
			f.write(jsondata)
		f.closed
