			    	...
			    	if company == "Verizon":
				    	...
				    	for shapeFile in os.listdir(shapeFileDir):
		  				...
		  				# 				 "CONSTRAINT entity_check CHECK (entity::text = '" + company + "')) " + \
								...
								sqlComm = "create table swat.mytemp1 (geom geometry);"
								cursor.execute(sqlComm)
								conn.commit()
								...
								#while len(gids)>0:
									...
									for gid in gids:
										...
										sqlComm = sqlComm + "insert into swat.mytemp1 (geom) select CASE WHEN st_geometrytype(geom) like '%Polygon' Then geom else st_buffer(geom,0.0) end from swat.mytemp;"
