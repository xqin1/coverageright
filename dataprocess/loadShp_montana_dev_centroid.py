...
def main():
	try:
		...
		dbname = "gisdb" 
		schema = "swat"
		tableName = "coverageright201210_montana_centroid"
		user = "bbmap"
		password = "bbmap_ps"
		...
		conn.commit()
		sqlComm = "CREATE TABLE " + schema + "." + tableName + "(geoid10 varchar(100),entity varchar(100), protocol varchar(100))"
		cursor.execute(sqlComm)
		conn.commit()
		...
		for techType in os.listdir(rootDir):
		    ...
		    for company in os.listdir(techTypeDir):
		    	...
		    	for shapeFile in os.listdir(shapeFileDir):
					if shapeFile.endswith("shp") and shapeFile.find('centroid')>0:
						...
						if companyCount>0:
							print companyCount
							shpComm = 'shp2pgsql -s 4326 -d -I -W latin1 -g geom "' + shapeFileDir + "\\" + shapeFile + '" ' + schema + "." + shapeFile.replace("_dissolved_montana_centroid.shp","") + " | psql -h " + host + " -d " + dbname + " -U " + user
							print "load " + shapeFile + " to postgres"
							shpP = subprocess.call(shpComm,shell=True)
							#shpComm = 'alter table '
							sqlComm = "insert into " + schema + "." + tableName + " select geoid10, '" + company + "','" + techType + "' from " +  schema + "." + shapeFile.replace("_dissolved_montana_centroid.shp","")
							print sqlComm
							cursor.execute(sqlComm)
							conn.commit()
							sqlComm = "drop table if EXISTS " + schema + "." + shapeFile.replace("_dissolved_montana_centroid.shp","") + " CASCADE"
							cursor.execute(sqlComm)
							conn.commit()		
		print finalList
		...
		with open("c:\\projects\\python\\coverageRigh201210_dev_montana_centroid.json" , 'w') as f:
			f.write(jsondata)
		f.closed
